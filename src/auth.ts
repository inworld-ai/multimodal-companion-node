...
function generateSignatureCSharpCompatible(
  values: string[],
  secret: string
): string {
...
function getClientCredsFromInworldEnv(): {
  apiKey: string;
  apiSecret: string;
} | null {
...
export function verifyIncomingRequest(req: IncomingMessage | Request): {
  ok: boolean;
  error?: string;
} {
  ...
  if (!creds)
    return {
      ok: false,
      error:
        'Server not configured for client auth (INWORLD_API_KEY missing/invalid)',
    };
  if (parsed.apiKey !== creds.apiKey)
    return { ok: false, error: 'ApiKey mismatch' };
  if (!isDateTimeSkewAcceptable(parsed.dateTime))
    return { ok: false, error: 'DateTime skew too large' };

  // check nonce replay
  const now = Date.now();
  cleanupExpiredNonces(now);
  if (nonceTimestamps.has(parsed.nonce))
